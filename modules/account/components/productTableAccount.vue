        ...
        <div class="table-responsive">
            <table class="table">
                <thead>
                    <tr>
                        ...
                        <th>จำนวน</th>
                        <th>ราคาหน้าเว็บ</th>
                        <th>ราคาที่ต่อรองได้</th>
                        <th>ราคาขาย</th>
                    </tr>
                </thead>
                <tbody v-if="accountStore.ticket.ticketProducts.length > 0">
                    <template v-for="(data, key) in accountStore.ticket.ticketProducts" :key='key'>
                        <tr v-if="data.ticketProductItems.length == 1">
                            ...
                            <td>{{data.ticketProductItems[0].priceAfterDiscount.toFixed(2)}}</td>
                            <td>
                                <input
                                    type="number"
                                    ...
                                    @blur="priceAfterDiscountBlur($event)"
                                    v-model="data.ticketProductItems[0].priceFinal"></td>
                            </tr>
                            <template v-else>
                                <tr>
                                    ...
                                    <tr v-for="(data2, key2) in data.ticketProductItems.slice(1)" :key='key2'>
                                        ...
                                        <td>{{data2.priceAfterDiscount.toFixed(2)}}</td>
                                        <td>
                                            <input
                                                type="number"
                                                class="form-control form-control-sm"
            ...
            <div class="col">
                ...
                &emsp;<span>ราคารวม {{ totalPrice() }} หยวน</span>
                    ...
                    <div
                        class="modal fade"
                        id="salesCTModalProduct"
                        tabindex="-1"
                        aria-labelledby="salesCTModalProduct"
                            ...
                            <div class="modal-content">
                                ...
                                <Form @submit="submitProduct" ref="formProduct">
                                    <div class="modal-body">
                                        <div v-for="(input, index) in obj.data.productLink" :key="index">
                                            <div class="form-group row">
                                                <Field
                                                    ...
                                                    <div class="col">
                                                        <button
                                                            v-show="index == obj.data.productLink.length-1"
                                                            type="button"
                                                        ...
                                                        <button
                                                            v-show="index || ( !index && obj.data.productLink.length > 1)"
                                                            type="button"
                                        ...
                                        <div class="form-group row">
                                            <Field
                                                ...
                                                label="ประเภทสินค้า"
                                                v-slot="{ field, errors }"
                                                v-model="obj.data.productTypeId">
                                        ...
                                        <div v-for="(input, index) in obj.data.ticketProductItems" :key="index">
                                            <div class="form-group row">
                                                ...
                                                <div class="col-4 col-sm-3">
                                                    <Field
                                                        :name="'productSpecification'+index"
                                                        rules="required"
                                                ...
                                                <div class="col-3 col-sm-2">
                                                    <Field
                                                        :name="'priceFromSystem'+index"
                                                        rules="required|min_value:0"
                                                        label="ราคา"
                                                        v-slot="{ field, errors }"
                                                        v-model="input.priceFromSystem">
                                                        <input
                                                            :class="['form-control', errors[0] ? 'is-invalid' : '']"
                                                            v-bind="field"
                                                            type="text"
                                                ...
                                                <div class="col">
                                                    <button
                                                        v-show="index == obj.data.ticketProductItems.length-1"
                                                        type="button"
                                                        class="btn btn-outline-dark mr-2"
                                                        @click="addProductOption(index)">
                                                        <i class="fas fa-plus"></i>
                                                    </button>
                                                    <button
                                                        v-show="index || ( !index && obj.data.ticketProductItems.length > 1)"
                                                        type="button"
                                                        class="btn btn-outline-danger"
                                    ...
                                    <div class="modal-footer">
                                        <button type="submit" class="btn btn-primary">{{ modalType == 'create' ? 'เพิ่ม' : 'แก้ไข' }}</button>
                                        <button
                                            type="button"
                ...
                export default defineComponent({
                    ...
                    setup() {
                        ...
                        function submitProduct(data : any, actions : any) {
                            ...
                            let jQueryObj : any = Object
                                .keys(modal)
                                .filter((key) => (key.toString().indexOf('jQuery') !== - 1) && modal[key].hasOwnProperty('bs.modal'))
                            if (modalType.value == 'create') {
                                obj.data.productNumber = Date
                                    .now()
                                    .toString()
                                let newObj = JSON.parse(JSON.stringify(obj.data));
                                accountStore.addProduct(newObj)
                                ...
                                let newObj = JSON.parse(JSON.stringify(obj.data));
                                accountStore.editTicketProducts(newObj)
                        ...
                        function editProduct(id : any) {
                            ...
                            let product = accountStore.getTicketProductsById(id)
                            setTimeout(() => {
                        ...
                        function deleteProduct(id : any) {
                            Swal
                                .fire({title: 'Confirm delete?', icon: 'warning', confirmButtonText: 'Delete', showCancelButton: true, confirmButtonColor: '#d33'})
                                .then(async(result) => {
                                    if (result.isConfirmed) {
                                        accountStore.deleteProduct(id)
                        ...
                        function addProductOption(index : any) {
                            obj
                                .data
                                .ticketProductItems
                                .push({productSpecification: "", qty: null, priceFromSystem: null, priceAfterDiscount: 0, priceFinal: 0});
                        }
                        ...
                        function totalQty() {
                            ...
                            accountStore.setTotalQty(total)
                            return total
                        }
